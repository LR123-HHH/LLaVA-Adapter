...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, image_sizes=None, sam_out=None, max_patch_per_group=5
    ):
        ...
        # image_features shape [bs,patch_nums*patch_nums,hidden_size]
        # sam_out = [torch.randint(0, 5, (24*24,))for _ in range(2)]
        if sam_out is not None: 
            new_image_features = []
            for batch_idx, single_image_features in enumerate(image_features):
                sam_labels = sam_out[batch_idx] 
                unique_labels = torch.unique(sam_labels) 
                ...
                for label in unique_labels: 
                    ...
                    indices = (sam_labels == label).nonzero(
                        as_tuple=True)[0] 
                    num_patches = len(indices)

                    for i in range(0, num_patches, max_patch_per_group):
                        group_indices = indices[i:i + max_patch_per_group]
                        group_features = single_image_features[group_indices]

                        merged_feature = torch.mean(
                            group_features, dim=0) 
                ...
                if merged_features:
                    ...
                    new_image_features.append(merged_features)
                else:
                    new_image_features.append(single_image_features)

            # new_image_features
            max_len = max(x.shape[0] for x in new_image_features) 
            padded_image_features = []
            for features in new_image_features:
                cur_len = features.shape[0] 
                
                padding = torch.zeros(
                    (max_len - cur_len, features.shape[1]), dtype=features.dtype, device=features.device)
                padded_features = torch.cat([features, padding], dim=0)
                padded_image_features.append(padded_features)
            image_features = torch.stack(padded_image_features, dim=0)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # =======================
            cur_image_features = image_features[cur_image_idx]
            text_embeds = cur_input_embeds

            attention_scores = torch.matmul(
                cur_image_features, text_embeds.transpose(0, 1))
            attention_weights = torch.softmax(
                attention_scores, dim=-1)

            cur_image_features = torch.matmul(
                attention_weights, text_embeds)

            # image_features[cur_image_idx] = weighted_image_features
            # =======================
